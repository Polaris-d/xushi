...
- xushi-skills：面向 Codex 等 agent 工具的 skill 包，指导 agent 选择任务类型、追问缺失信息并生成 xushi task JSON。
...
- `skills/xushi-skills`：agent skill，包含核心工作流、任务类型指南、schema 模板、澄清问题清单和优化反馈记录模板。
...
- `runs`：运行记录 payload、任务 ID、调度时间、状态。运行状态包括 `succeeded`、`failed`、`pending_confirmation`、`following_up` 和 `cancelled`。
...
- 注册工具：`xushi_health`、`xushi_create_task`、`xushi_list_tasks`、`xushi_get_task`、`xushi_trigger_task`、`xushi_list_runs`、`xushi_confirm_run`、`xushi_confirm_latest_run`、`xushi_callback_run`、`xushi_list_executors`、`xushi_install_hint`。
...
- 安装脚本支持 `XUSHI_INSTALL_AGENT_SKILLS=codex` 或 `--agent-skills codex`，在 agent 已经征得用户同意时非交互式安装 `xushi-skills`。Codex 目标默认写入 `$CODEX_HOME/skills/xushi-skills`，未设置 `CODEX_HOME` 时写入 `~/.codex/skills/xushi-skills`；已有同名 skill 时先备份为带时间戳的目录。
- `xushi-skills` 将喝水、起立、伸展、眼休息等健康习惯默认映射为 `recurring + anchor=completion + requires_confirmation=true`，同时提供 `references/optimization-notes.md`，要求 agent 把真实使用中的问题、前因后果、预期效果和可行动方向记录为本地反馈草稿，不自动上传。
