...
- `xushi.capabilities`：CLI、HTTP API 和插件工具共享的 agent 能力发现清单。
- `xushi.service`：应用服务层。
- `xushi.lifecycle`：运行记录成功确认、callback 和 follow-up 级联状态流转。
- `xushi.metrics`：进程内轻量指标和最近 tick 摘要。
...
- `GET /api/v1/health`
- `GET /api/v1/capabilities` 返回 agent 能力发现清单，不需要鉴权。
- `POST /api/v1/tasks`
- `GET /api/v1/tasks` 支持 `limit` 查询参数，默认 100。
- `GET /api/v1/tasks/{id}`
...
- `GET /api/v1/runs` 支持 `task_id`、`status`、`active_only` 和 `limit` 查询参数，默认 100。
- `POST /api/v1/runs/{id}/confirm`
- `POST /api/v1/runs/{id}/callback`
- `GET /api/v1/notifications`
- `GET /api/v1/deliveries` 支持 `limit` 查询参数，默认 100。
...
`GET /api/v1/capabilities` 的 `data.entrypoints` 同时给出 HTTP、CLI 和 OpenClaw 插件的自发现入口；`data.capabilities` 中每个能力包含用途、HTTP 方法与路径、CLI 命令和插件工具名。HTTP-only agent 可先读取该接口或 FastAPI 自动生成的 `/openapi.json`，CLI-only agent 可先运行 `xushi capabilities`，避免把 `trigger` 误当作完成确认。
...
SQLite 连接按操作短连接打开并立即关闭，避免 Windows 下 daemon、测试或安装器清理数据文件时遇到文件句柄占用。`sqlite_journal_mode` 默认 `delete`，可配置为 `wal` 提升读写并发；`sqlite_synchronous` 默认 `full`，可配置为 `normal` 或 `off`，但需要用户接受对应的一致性权衡。
...
Executor 是本地配置，不属于运行态数据。`Settings.executors` 从 `config.json` 的 `executors` 数组加载，service 在启动时构建启用 executor 映射。修改 executor、全局 `quiet_policy`、`reminder_aggregation` 或自动重试策略后可调用 `POST /api/v1/config/reload` 显式重新读取配置；reload 只替换内存中的 executor 列表、启用 executor 映射、`QuietPolicyEngine`、普通提醒聚合配置和自动重试参数，不切换数据库路径、SQLite PRAGMA、监听地址、端口、API token 或调度间隔。配置解析失败时 API 返回错误并保留旧运行时配置。`GET /api/v1/executors` 用于查看当前 daemon 运行时加载的 executor，CLI `xushi executors` 用于查看本地配置文件解析结果；两者都不提供写入接口，避免 agent 在运行时把投递凭据或外部入口写入 SQLite。
...
`Run` 使用 `origin_run_id` 将跟进记录关联到原始运行记录。用户或 agent 确认任一跟进记录后，序时会同步确认原始运行记录，并将同源仍在 `following_up` 或 `pending_confirmation` 的跟进记录标记为 `cancelled`。确认原始运行记录或归档任务时，也会取消关联的待处理跟进，避免历史 run 干扰 agent 判断。用户在提醒到点前主动完成 completion anchor 循环任务时，服务层会创建一条 `result.manual_completion=true` 的主 run，状态直接为 `succeeded`，`confirmed_at` 等于用户完成时间，不创建 delivery，用作下一次调度的完成锚点。
...
`complete` 是更通用的按任务完成入口。若任务存在未完成主 run，服务层会确认该 run 并取消关联的待投递 delivery 或跟进；若当前没有可确认主 run，且任务是 `recurring + anchor=completion`，则写入手动完成锚点 run，不触发提醒。HTTP-only agent 使用 `POST /api/v1/tasks/{id}/complete`，CLI-only agent 使用 `xushi complete <task_id>`，OpenClaw agent 使用 `xushi_complete_task`。`trigger` 仍只表示测试或立即执行，不用于完成确认。
...
- `parse_iso_duration` 支持 `P1D`、`PT10M`、`P1DT2H30M` 等天/时/分/秒组合；年月、小数和负数 duration 不具备稳定本地日历语义，统一拒绝。
- `anchor=completion` 的循环任务在上一条主运行记录确认完成后，基于确认时间重新计算下一次 RRULE 发生时间；未确认时不会继续生成主运行。服务层要求 `completion` 任务启用 `requires_confirmation`，并在任务更新时阻止从非 completion anchor 切换到缺少确认锚点的终态历史运行记录，避免调度器进入无后续 run 的隐式状态。用户提前完成且没有待确认主 run 时，`complete` 创建手动完成锚点 run，让下一次 RRULE 从这次实际完成时间重新展开，而不是预创建未来提醒 run。
...
- scheduler 只负责判断任务到期并生成 `Run`；delivery 层负责判断是否立即投递、延迟到免打扰结束、同分钟轻量聚合、聚合摘要、跳过、静默或绕过。
...
- `reminder_aggregation` 是普通提醒轻量聚合配置，默认启用同分钟 `pending` delivery 聚合。delivery 层按 `action.type` 和 `executor_id/system` 分组，只有同一时间窗口内至少 `min_items` 条普通提醒才创建 digest delivery；原始 delivery 标记为 `digested`，原始 run 保持可审计和可确认。显式 `quiet_policy.mode=bypass`、配置 `expiry` 的时效敏感任务和非 `pending` delivery 不参与普通聚合。
...
- 可选自动重试由 `auto_retry_failed_deliveries` 控制，默认关闭。开启后 daemon 在 tick 末尾为仍对应未完成 run 的失败 delivery 创建 retry，`auto_retry_max_attempts` 限制每条 retry 链的自动重试次数，新 delivery 的 `result.auto_retry_attempts` 记录次数。
- FastAPI lifespan 启动后台调度循环，默认每 30 秒执行一次到期任务和跟进扫描。调度循环启动时输出 info 日志；每次 tick 创建 run 时输出摘要，空 tick 使用 debug 日志。
- `GET /api/v1/metrics` 返回进程内计数器和最近 20 次 tick 摘要，用于 doctor、agent 或本地监控判断 daemon 是否在持续工作。指标随进程重启清零，不作为审计事实来源。
...
- 注册工具：`xushi_capabilities`、`xushi_health`、`xushi_create_task`、`xushi_list_tasks`、`xushi_get_task`、`xushi_update_task`、`xushi_delete_task`、`xushi_trigger_task`、`xushi_complete_task`、`xushi_list_runs`、`xushi_list_notifications`、`xushi_list_deliveries`、`xushi_retry_deliveries`、`xushi_reload_config`、`xushi_confirm_run`、`xushi_confirm_latest_run`、`xushi_callback_run`、`xushi_list_executors`、`xushi_install_hint`。
...
- `xushi plugins install` 采用复制式安装，不执行 npm 依赖安装；因此插件运行入口只保留 OpenClaw 宿主 SDK peer dependency，工具参数 schema 使用插件内置的轻量 JSON schema helper 生成，避免安装后缺少普通 npm runtime dependency。
...
- `.github/workflows/build.yml` 在 Windows、macOS、Linux 上执行测试、项目元数据一致性检查、ruff、wheel 构建和二进制构建，并上传已按平台重命名的构建产物，不上传 PyInstaller 临时 `.spec` 文件。
- `scripts/check_project_metadata.py` 检查 `pyproject.toml`、OpenClaw 插件 manifest/package 版本号，以及内置 plugin/skills 副本与仓库源码是否一致。
...
- `.github/workflows/release.yml` 在 `v*` tag 上先运行跨平台质量检查，再分别构建 Python 包和平台二进制，最后合并 release 资产、生成 `SHA256SUMS.txt` 并发布 GitHub Release 自动说明。build/release 工作流通过 `UV_PYTHON=3.12` 固定 PyInstaller 打包解释器，Linux 二进制在 Ubuntu 22.04 构建以扩大 glibc 兼容面，并在上传资产前执行 `xushi --help` 与 `xushi-daemon --help` 启动 smoke test；smoke test 使用短超时保护，Windows 步骤启用 PowerShell 原生命令错误传播；release 工作流额外在 Ubuntu 26.04 用户态容器中执行 Linux 二进制启动测试。
- `xushi upgrade status/check/backup/apply/rollback` 提供用户手动触发的安全升级能力。升级器默认不做静默自动升级；`apply` 先检查 daemon 端口，再备份配置和 SQLite 数据库，随后从 GitHub Release 下载目标版本或 latest 的 `xushi-<platform>` 与 `xushi-daemon-<platform>`，替换用户级全局命令目录中的二进制，并在成功 JSON 中输出 `post_upgrade_notice` 提醒执行升级后投递测试。
- 安装和升级流程的最终验收都必须是同一条真实投递链路测试：运行配置检查，确认 daemon 使用预期版本和配置，创建唯一 smoke-test reminder，并由用户确认日常目标渠道实际收到消息。升级后若曾安装 OpenClaw 插件或 `xushi-skills`，需先用新版本程序同步内置副本，再测试投递链路。
...
| 2026-05-11 | 新增 | 增加运行期 metrics、RunLifecycleService、有限自动重试、SQLite PRAGMA 配置、列表默认上限和元数据一致性 CI。 |
| 2026-05-11 | 明确 | 服务层新增 completion anchor 配置和迁移校验，禁止从无确认终态 run 推断完成锚点。 |
| 2026-05-11 | 调整 | 发布工作流固定 Python 3.12 构建二进制，并增加跨平台和 Ubuntu 26.04 用户态启动验证。 |
| 2026-05-12 | 更正 | 修复发布 smoke test 挂起风险，补充 daemon help 退出、Windows UTF-8 输出和短超时保护。 |
| 2026-05-12 | 新增 | 增加 `xushi.capabilities`、HTTP/CLI/OpenClaw 能力发现入口，并补齐 CLI 与插件的常用任务和确认工具。 |
| 2026-05-12 | 明确 | 安装和升级完成验收统一要求真实投递测试，并要求升级后同步内置插件/skills 后再验证链路。 |
| 2026-05-12 | 新增 | `xushi upgrade apply` 成功结果增加 `post_upgrade_notice`，直接提示用户或 agent 执行升级后测试。 |
| 2026-05-12 | 新增 | 增加 `complete` 按任务记录完成入口，支持提前完成 completion anchor 循环任务并创建不投递的手动完成锚点 run。 |
| 2026-05-14 | 新增 | 增加普通提醒同分钟轻量聚合设计，复用 digest delivery 保留审计语义。 |
| 2026-05-14 | 更正 | 移除 OpenClaw 插件对 `@sinclair/typebox` 的普通运行时依赖，确保内置复制式安装后可由宿主直接加载。 |
