...
- 预编译二进制必须使用稳定 Python 版本构建，并在发布工作流中执行有短超时保护的启动级 smoke test，避免打包运行时与目标系统内核策略不兼容或常驻进程导致发布长时间挂起。
...
- 通过 `xushi plugins install` 安装的内置 OpenClaw 插件必须可直接由宿主加载，不得依赖安装流程额外执行 `npm install`；除 OpenClaw 宿主 SDK 外，插件运行入口不得声明未随插件复制的普通 npm 运行时依赖。
...
| 2026-05-14 | 更正 | 明确内置 OpenClaw 插件安装后必须零普通 npm 运行时依赖，避免复制式安装缺少依赖包。 |
