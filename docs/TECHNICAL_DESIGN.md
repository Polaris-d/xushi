...
- 失败 delivery 不自动无限重试，避免配置错误时持续打扰外部系统；用户修复 executor token、URL、TLS 或 agent 路由配置并重启 daemon 后，可通过 `POST /api/v1/deliveries/retry` 或 CLI `xushi retry-deliveries` 创建新的 retry delivery。原失败 delivery 保留为审计历史，新的 delivery 通过 `result.retry_of` 指向原记录，run 会回到 `pending_delivery` 后立即重新投递。
...
- 注册工具：`xushi_health`、`xushi_create_task`、`xushi_list_tasks`、`xushi_get_task`、`xushi_trigger_task`、`xushi_list_runs`、`xushi_list_deliveries`、`xushi_retry_deliveries`、`xushi_confirm_run`、`xushi_confirm_latest_run`、`xushi_callback_run`、`xushi_list_executors`、`xushi_install_hint`。
