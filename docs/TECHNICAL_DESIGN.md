...
- `anchor=completion` 的循环任务在上一条主运行记录确认完成后，基于确认时间重新计算下一次 RRULE 发生时间；未确认时不会继续生成主运行。服务层要求 `completion` 任务启用 `requires_confirmation`，并在任务更新时阻止从非 completion anchor 切换到缺少确认锚点的终态历史运行记录，避免调度器进入无后续 run 的隐式状态。
...
- 注册工具：`xushi_capabilities`、`xushi_health`、`xushi_create_task`、`xushi_list_tasks`、`xushi_get_task`、`xushi_update_task`、`xushi_delete_task`、`xushi_trigger_task`、`xushi_list_runs`、`xushi_list_notifications`、`xushi_list_deliveries`、`xushi_retry_deliveries`、`xushi_reload_config`、`xushi_confirm_run`、`xushi_confirm_latest_run`、`xushi_callback_run`、`xushi_list_executors`、`xushi_install_hint`。
